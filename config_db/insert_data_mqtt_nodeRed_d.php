    <?php
        ...
        if(isset($_POST['data_array'])) {
            ...
            if($S_number == "KMUMT001" || $S_number == "KMUWE001" || $S_number == "KMUWE002" || $S_number == "OVTMT001"){
                ...
                $select = $dbcon->prepare("SELECT * FROM tb_data_sensor WHERE data_sn = '$S_number' ORDER BY data_id DESC LIMIT 1 ");
                $select->execute();
                ...
                // exit();
                if($time_D == $time_new ){
                    echo json_encode(['status' => "ข้อมูลซ้ำ",'data' => "ไม่บันทึก","SN" => $json->serial_id ], JSON_UNESCAPED_UNICODE );
                }else{
                    ...
                    try{
                        ...
                        if ($dbcon->prepare($sql)->execute($data) === TRUE) {
                            echo json_encode(['status' => "OK",'data' => "Insert_Success","SN" => $json->serial_id ], JSON_UNESCAPED_UNICODE );
                        }else{
        ...
        // master
        elseif(isset($_POST["data_array2"])){
