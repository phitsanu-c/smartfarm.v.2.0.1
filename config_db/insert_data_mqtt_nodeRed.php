<?php
    ...
    // -----------------------------------------------------------------------
    // echo json_encode($_REQUEST['data_array']);
    ...
    if(isset($_POST['data_array'])) {
        ...
        // $SQL_house = $dbcon->prepare("SELECT * FROM tb2_house WHERE house_master = '$S_number' order by house_id ");
        // $SQL_house->execute();
        // $row_selHO = $SQL_house->fetch(PDO::FETCH_ASSOC);
        // $house_siteID = $row_selHO["house_siteID"];
        // $house_id = $row_selHO["house_id"];

        // $house_siteID = $_POST["house_siteID"];
        // $house_id = $_POST["house_id"];
        // echo "house_siteID : ".$house_siteID." and house_id : ".$house_id;
        // echo $_POST["id"];
        // exit();
        $select = $dbcon->prepare("SELECT data_timestamp FROM tb_data_sensor WHERE data_sn = '$S_number' ORDER BY data_id DESC LIMIT 1 ");
        $select->execute();
        $row_select = $select->fetch(PDO::FETCH_ASSOC);
        $row_count = $select->rowCount();
        // echo $row_count; exit();
        
        // exit();
        if($row_count != 0){
            $time_D = substr( $row_select['data_timestamp'] ,0 , 18);
            $time_new = substr( $json->date_time, 0, 18);
            if($time_D == $time_new ){
                echo json_encode(['status' => "sum",'data' => "not insert",'time_db'=>$time_D,'time_new'=>$time_new,"SN" => $json->serial_id ] , JSON_UNESCAPED_UNICODE );
                exit();
            }
        }
        ...
        try{
            ...
            if ($dbcon->prepare($sql)->execute($data) === TRUE) {
                echo json_encode(['status' => "OK",'data' => "Insert_Success",'time_new'=>$time_new,"SN" => $json->serial_id ] , JSON_UNESCAPED_UNICODE );
            }else{
