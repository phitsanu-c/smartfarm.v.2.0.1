<?php
    ...
    if ($_POST['mode_report'] == 're_sensor') { // re_sensor
        ...
        for($i=0; $i < $count_columns; $i++){
            if ($config_cn[3][$i] >= 4 && $config_cn[3][$i] <= 7) {
        ...
        $sql = "SELECT $channel1 FROM $tb_name WHERE data_sn = '$house_master2' AND data_timestamp BETWEEN '$start_day' AND '$stop_day' AND mod(minute(`data_timestamp`),'$sel_all_every') = 0 ORDER BY data_timestamp DESC";
        // echo $sql;
        // exit();
        $stmt = $dbcon->query($sql);
        $data0 = array();
