...
<div class="page-content">
    ...
    <?php
        ...
        if ($_SESSION["count_statusUser"] == "supperadmin") {
            $site_stmt = $dbcon->query("SELECT * FROM tbn_userst INNER JOIN tbn_house ON tbn_userst.userST_houseID = tbn_house.house_id WHERE tbn_userst.userST_siteID = '$siteID' GROUP BY `userST_houseID` ");
        } else if ($_SESSION["count_statusUser"] >1) {
            $site_stmt = $dbcon->query("SELECT * FROM tbn_userst INNER JOIN tbn_house ON tbn_userst.userST_houseID = tbn_house.house_id WHERE  tbn_userst.userST_accountID='$siteID' AND tbn_userst.userST_siteID = '$siteID' GROUP BY `userST_houseID` ");
        }
        $i = 1;
        // $row_ = $site_stmt->fetch(PDO::FETCH_BOTH);
        foreach ($site_stmt as $row_) {
            // echo $row_["userST_siteID"].','.$row_["house_master"];
    ?>
        <div class="col-12 col-sm-12 col-md-4 col-lg-4  col-xl-3">
            <a href="<?php if(substr($row_["house_master"],0,2) == "TUS"){ echo $url_link .'/tu/#'.encode($row_["userST_siteID"].','.$row_["house_master"]);}else{echo $url_link .'#'.encode($row_["userST_siteID"].','.$row_["house_master"]);} ?>">
