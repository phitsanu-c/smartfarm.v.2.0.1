...
<div class="page-content">
    ...
    <?php
        ...
        if ($_SESSION["count_statusUser"] == "supperadmin") {
            $site_stmt = $dbcon->query("SELECT * FROM `tbn_userst` INNER JOIN tbn_site ON tbn_userst.userST_siteID = tbn_site.site_id INNER JOIN tbn_house ON tbn_userst.userST_houseID = tbn_house.house_id GROUP BY userST_siteID ");
        } else {
        ...
        $co = $row_["site_id"];
        $count_house = $dbcon->query("SELECT count(userST_houseID) FROM tbn_userst WHERE userST_accountID = '$accountID' AND userST_siteID ='$co'")->fetch(PDO::FETCH_BOTH);
    ?>
